...
if mode == "b":
    ...
    for i,item in enumerate(file_list):
        ...
        output, final_array = app_rula(item)
        f_name = item.split('\\')[-1]
    ...
    while True:
        try:
            ...
            output, final_array = app_rula(path)
            write_to_file(fObj, output)

            rp.rula_to_pdf("test", final_array)
            end_time = dt.datetime.now()
            time_delta = end_time - start_time
            print("Process Duration: %1.4f" %time_delta.total_seconds())
